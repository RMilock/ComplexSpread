...
def plot_sir(G, ax, D = None, beta = 1e-3, mu = 0.05, start_inf = 10, numb_iter = 100):
  ...
  leg = ax.legend(bbox_to_anchor=(1, 1), edgecolor="dimgrey", loc='upper left')
  ...
  ax2 = fig.add_axes([leg_pxls.x0/fig_pxls.width,
                      ...
                      (leg_pxls.y0-ax_pxls.y0)/fig_pxls.height-pad/4])

  # eliminating all the tick marks:
  ax2.tick_params(axis='both', left=False, top=False, right=False,
                  bottom=False, labelleft=False, labeltop=False,
                  labelright=False, labelbottom=False)
  
  # adding some text:
  ax2.text(0.1,0.8,textstr)


def plot_G_degdist_adjmat_sir(G, p = 0, D = None,  numb_iter = 500, beta = 1e-3, mu = 0.05, \
  ...
  if plot_all == False: _, ax = plt.subplots(figsize = figsize, constrained_layout = True)
