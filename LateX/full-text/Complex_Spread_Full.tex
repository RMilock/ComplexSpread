...
Finally, since \autoref{eqs:SIR_degree-based} are not linear, there is a strong dependence on the initial conditions: an epidemic reaching $ 149$ individuals starting with $ 10$ infected could have a different trend than the one that diffuses in $ 15$ nodes with $ 1$ initial contagion.
...
\label{sec:tmax-sigmamax_Nontrivialdep}
The duration of an average of different epidemics $t_{max}$ seems to increase as the average of the fraction of total cases $\pi(t) \sim 1/2$. This will be shown shortly but could be plausible since, for $ \pi(t) \sim 1/2$, a plethora of behaviors are allowed and, thus, a particular trajectory could last more.
Rather, whenever the total cases is near $ 0$, many scenarios accounts for a fast decaying pathogen; while wheter $ \pi(t) \sim 1$ the outbreak is strong enough to reach all the population in a short time. See \autoref{sec:res_OPSW}. 

The variance $\sigma(t_{max})$ embodies the "degree of stochasticity" of the trajectories displayed in pale green (network-based) and brown (mean-field): it is reported after the $\pm$ sign in the legend. Its behavior is similar to the one of $t_{max}$. For a in-depth analysis, see \autoref{sec:res_OPSW}.
...
\chapter{Results}
\label{ch:Results}
In the following sections, after a preliminary analysis on the \textit{basic reproduction number} $R_0$, we introduced the \textit{epidemic severity} $ \Delta R_0 (\delta)$ to account for the burdening of a pathogen within a population.
Subsequently, we discuss the evolution of a disease (see \autoref{sec:degree-basedMF}) on different graphs (see \autoref{ch:network-models}). In particular, we generated one version of each network model with only local connections, namely with $ p = 0$, and a second one with long-range interactions, i.e. $ p = 0.3$. 
Thus, we simulated the COVID-19 spreading, by fixing $\beta = 0.015 \textnormal{ and } d = 14$ \cite{Thurner::NetBasedExpl}. In addition, we obtained two scenarios by chosing, one at a time, the transmissibility $\beta = 0.1$ and the recovery days $ d = 4$ while we fixed the average contacts $D$ and the distant fraction of nodes $p$ to the COVID-19 curves. Ultimately, we compared the different evolutions of the pathogens, in order to understand the interplay among the disease and the underlying social network.
...
This review provides that the Caveman model is the best suited topology in costraining the diffusion of a pathogen among the studied ones. Indeed, for $ \beta = 0.1$ a large fraction of the population is instead infected. Thus, it drives to the idea that, in some cases, only major restriction are effective, e.g. forced quaratines.
The other networks have less restricting power since they are not made of clusters. More precisely, we organized the nodes on a circle and forced the node degrees to connects to its "neighbors" according to a defined scheme. In turn, the latter procedure generate a \textit{fan-shape} structure which strongly overlaps as $D$ increases. Therefore, we recovered that 
...
As the following part will be a technical description of the evolutions of a virus, we suggest to firstly dive into \autoref{ch:Methodology} for a more general discussion on the introduced quantities and behaviors.
...
\end{figure}
The goal of this section is to test whether the \textit{basic reproduction number} $R_0$ is a suitable quantity to estimate the "strength" of a diffusion among different network models. In particular, $R_0$ correctly estimates the number of secondary cases only at the beginning of the epidemic and worsens as the time grows. 
Due to this fact, we propose a new quantity called the \textit{epidemic severity}  $\Delta R_0(\delta):= \frac{R_0 - R_{c-net}}{\delta}$ which accounts for a kind of network "efficiency" $1/\delta$. Indeed, a low value of the average path length $ \delta$  drives to a higher spreading of a pathogen. 
\\Therefore, $\Delta R_0(\delta)$ seems to better describe how a pathogen seeps into a population, e.g. by comparing its value among the \autoref{fig:sir_RegLat_D14} and \autoref{fig:sir_RegLat_D6_p0.3} .

The work-flow is the following. We generate a fully-connected graph using a "Watts-Strogatz" (WS) algorithm with $D = 999, p=0$. This model is, thus, called "Regular Lattice Graph". Then, the core idea is to fix $ R_0 $ and visualize how the epidemic curves change by varying the transmissibility $ \beta$ and $ D$. In particular, we fixed the recovery rate $ \mu = 1/d$ and, then, dividing the average number of contacts $ D$ while doubling $\beta$. 
Formally,
\begin{equation}
...
\begin{figure}[ht]
	...
	\centering
	\caption{Network SIR model (green) benchmarked with the Annealed-Mean-Field Network (orange) for $R_0 > R_{c-net}$ \autoref{eq:RcRnet_network}. A Caveman model with $ On-Ring-Links = 1$ \autoref{fig:sir_CM_D3_ORL1} exhibits an outbreak size of $1.6\%$ for a clique of $ D \sim 2.67$ which accounts for the household european size as in \cite{Householdsize:2020}. Hence, arranging the individuals into families, reduces the diffusion even if they communities are linked on a circle as in the current case.}
	\label{fig:sir_RegLat_D2_p0}
\end{figure}
In this subsection, the SIR curves for a local ( $ p = 0$ ) lattice graph are reported while compared with a Long-Range WS (see \autoref{sec:res_RegLat_p0.3}).
...
Secondly, by fixing $R_0 \sim 2.94$ while increasing $D$ , the curve in \autoref{fig:sir_RegLat_D61430} exhibit a growth in the final number of total cases $ \pi_{max}:= \pi(t_{max})$ where $ t_{max}$ is the time at the end of the infection, i.e. when the infected individuals vanishes, namely $i(t) = 0$.
In addition, \autoref{eq:lambdac_Rc_netSIR} implies that
...
Strengthening the relationship among nodes, namely increasing the average contacts for $D \geq 6$, \autoref{fig:sir_RegLat_D6} can capture the (pseudo-)linearity since the early stages of the contagion; while enhancing $\pi_{max}$. Indeed, for $ D = 6$ (\autoref{fig:sir_RegLat_D6}) the final outbreak is $ 3$ times higher than the $ D=2$ cases. Moreover $D = 6$, is slightly above the upper bound of $2-5$ lockdown contacts per person \cite{Liu::2021_Review_SContactPattern}. Hence, this could be the evolution of a small village where stay-at-home orders are light, as the network is not restricted into families.
...
By continuing enhancing $ D$, the network becomes more tangled, due to the increasing overlapping among  the \textit{"fan-shape" neighboorhood}. Thus, the epidemic diffuses more rapidly showing that the $ R_0$ is informative only to early times. Rather, $\Delta R_0(\delta)$ seems a more precise quantity to account for the epidemic severity, since it augments as $D$ increases.
This phenomenon, as said, is explictely described on the \autoref{fig:sir_RegLat_D14} caption, but could be compared with also other cases.

Finally, in \autoref{fig:sir_RegLat_D30}, the maximum of the infected $ i(t)$ ($\bigstar$) obtained by means of the \autoref{eq:picrit_network} overestimates the real maximum. On the way to recovering $ \pi(t_{critical})$, we made the following assumptions: \autoref{eq:RcRnet_network}, \autoref{eq:n(t)_fusemodel} to hold for every time. The former takes into consideration only quantities at early times. Thus, this divergence is somehow to be expected. The latter seems a fair approximation at every time.
...
The prediction worsens even more when the long-range wiring probability $ p$ is diffrent from zero, e.g. \autoref{fig:sir_RegLat_D6_p0.3}. 
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.9\linewidth}
        ...
        \caption{Regular Lattice with $D = 14$. Linearity is lost. Moreover, only $\Delta R_0(\delta)$ grasps the epidemic severity with respect \autoref{fig:sir_RegLat_D6_p0.3}. Indeed, the two $R_0$'s are equals; while the differences $ R_0 - R_{c-net} $ could suggest that the current case would be stronger that the $ p = 0$ case. Neverthless, it is the opposite as captured by $\Delta R_0(\delta)$.} 
    ...
    \begin{subfigure}[t]{0.9\linewidth}
        ...
        \caption{Regular Lattice with $D = 500$.} 
		\label{fig:sir_RegLat_D30}
    \end{subfigure}
    \caption{Some regular lattice spreading curves, which exhibit a strong dependence on $D$. As $D$ increases, the classical $R_0$ remains constant, while its (proposed) rescaled version ($R_0(\delta):=R_0 / \delta$) varies. The estimated maximum of $ i(t)$ ($ \bigstar$) is overestimated; while the maximum for a mean-field is correct (yellow star).}
...
As the probability of connecting to distant nodes is switched on, i.e. $p = 0.3$, the "Long-Range" Watts-Strogatz model starts to resemble a mean-field network where all the nodes are connected at random (see \autoref{fig:net_RegLat_D500_p0.3}). The SIR spreading for $ D = 6$ (\autoref{fig:sir_RegLat_D6_p0.3}) shows an increasing number of total cases $\pi_{max}|_{p=0.3} \sim 72\%$ against the $\pi_{max}|_{p=0} \sim 12\%$ of the regular case in \autoref{fig:sir_RegLat_D6}. This is, somehow, the aim from the \autoref{sec:ATechIntro}: the network really plays a role in constraining the disease, yielding a linear growth of the total infected even for big $R_0$ ($R_0 = 2.94$) but a small $ D=6$ as there could be the number of contacts in a light lockdown \cite{Liu::2021_Review_SContactPattern}.

As $D=14$ \autoref{fig:RegLat_D14_p0.3} or $ D = 2$, the fluctiation are smaller than the intermediate case for $ D = 6$. A rough explanation comes from the upper bound of population size $N$ and the lower one as an epidemic ends whenever $ i(t) = 0$. Indeed, those constraints even out the trajectories as it is briefly expressed in \autoref{ch:Methodology}. By constrast, wheter a pathogen has an "intermediate strength", between the mean-field and a fast decay evolution, its trajectories display broater scenarios, which translate into an higher fluctuation over the mean $ \sigma(t_{max})$ . 

Since for "Long-Range WS" the spreading is enanched, the take on message is that local wiring, and eventually social distancing, is effective. In turn, this could drive to a remarkable suppression of number of total cases $ \pi(t)$ if $ D$ is small enough.
...
\begin{figure}[p]
    ...
    \begin{subfigure}[t]{\textwidth}
        ...
        \caption{The (quasi-)linearity is lost and the exponential growth is recovered. By contrast to the $ D=2$ case, here the difference with respect to \autoref{fig:sir_RegLat_D6} is more pronunced: the total cases are barely $ 6$ times the "regular" ones. As quoted before, the estimated maximum (green star) worsens as $ p \neq 0$.} 
...
Therefore, to capture the absence of hubs while retaining the heterogeneity on the number of contacts, we introduced a poissonian small-world (PSW) network: the degree of each vertex is drawn from a poissonian distribution while the nodes are "properly" rewired only to the nearest nodes, in order to mimic a lockdown scenario \cite{Thurner::Appendix_NetBasedExpl}. Nevertheless, this arises the choice of whether to preserve locality (\autoref{fig:net_O-PSW}), obtaining a narrower distribution $p_k$, or to fix the poissonian distribution (\autoref{fig:net_S-PSW}), introducing some long-range connections. Based on the kind of connections, the former would be addressed as the "overlapping-PSW" (O-PSW); while the latter as the "sparse-PSW" (S-PSW). 

To gain a perceiving understanding of this, we drew each node degree for a poissonian distribution $ p_k$ with a defined average degree of contacts $D$. Then, we assumed that the nodes were arranged on a circle and we wire them anti-clock wise from the bottom. If the "locality" has to be preserved (\autoref{fig:net_O-PSW}), the wiring scheme establish that a node links to its nearest neighboring vertexes even if their degrees have been already saturated by previous wirings. Therefore, the poissonian distribution is lost. This kind of $ p_k$ resembles the Watts-Strogatz one for $ p_k \neq 0$ (\autoref{fig:net_RegLat_D500_p0.3}). The difference is that $ p \neq 0$ drives to a network with long-range interactions; while here only local nodes are reached.\\
On the other hand, by aiming at preserving $p_k$ (\autoref{fig:net_S-PSW}), the wiring procedure forces an individual to connect only to the nearest node, whose degree has not been saturated by the previous re-links. Thus, the target nodes could be distant from the source one. Since in \cite{Thurner::Appendix_NetBasedExpl}, there are reported some intriguing behaviors for a poissonian Small-World (PSW) but not which of the two kinds. Hence, we decided to examine both the O-PSW and S-PSW networks.

In order to simulate the COVID-19 pandemic,the transmissibility $\beta = 0.015$ and the incubation period $d = 14 \, ( \mu \sim 0.07)$ (see \autoref{App:muD_table}) are chosen from \cite{Thurner::NetBasedExpl} and \cite{LaurerSA:2020_IncPeriodCOVID-19}. As before, we analyze also the $d = 4$ case, choosing $\beta = 0.015$ but enanching $ D$ (see \autoref{fig:sir_O-PSW_D24_d4}).  In addition, we report the scenario where $ \beta = 0.1$ (a stronger epidemic) while fixing the other parameters to the ones in \autoref{fig:sir_O-PSW_D5_p0} (see \autoref{fig:sir_O-PSW_D5_d14_b0.1}).
...
As introduced in \autoref{sec:tmax-sigmamax_Nontrivialdep}, the maximum duration $ t_{max}$ of an epidemic and its variance over the mean $\sigma_{max}$ depend on the interplay among the network and the pathogen strength. Indeed, $ t_{max}$ and $\sigma_{max}$ are higher as the pathogen posses an "balanced" diffusion niether too weak as it will decaying fast, nor too strong by reaching shortly all the individuals. More specifically, the panel \autoref{fig:sir_O-PSW_COVID} shows a diminishing $ t_{max}$ as the epidemic reaches a small fraction of the population. Moreover, fixing $D$ while increasing $ \beta$, the outbreak time increases (cf. \autoref{fig:sir_O-PSW_D5_d14_b0.1}). The same phenomenon is recovered by changing $ p$ or $ \mu$  (cf. \autoref{fig:sir_O-PSW_D23_d4_p0.3}, \autoref{fig:sir_O-PSW_D24_d4}). 
On the other hand, the epidemic lasts less if the pathogen is too strong since it reaches the population in a smaller time \autoref{fig:sir_O-PSW_D5_p0.3_b0.1}. The $ \sigma_{max}$ depends on the underlying parameters, roughly, as $ t_{max}$. Specifically, by looking at \autoref{fig:sir_O-PSW_COVID}, for the figure in the middle different trends could develop throught time and $ \sigma(t_{max})$ is higher than in the other cases.
...
Finally, there would also shown the standard deviation of the daily cases $SD(C(t))$, namely the order parameter as a function of the average number of contacts $ D$, which is a proper way to highlight the separation among a sub-exponential regime and an exponential growth of the number of total cases $ \pi(t)$.

\subsubsection*{SIR on Regular O-PSW}
The triptych in \autoref{fig:sir_O-PSW_COVID} displays the appearance of the exponential growth of the fraction of total cases $\pi_{max}:= \pi( t_{max}) \to \infty$ as $D$ increases. Specifically, $ t_{max}$ is defined as the last time reported for a network evolution of the disease. Moreover, for small $D$, there is not an exponential growth, even at early times where there is a big pool of susceptible. Rather, this is the case for the Annealed-Mean-Field Network (orange curve) representing a mean-field approximation.

As in \autoref{sec:RegLat_p0}, by varying the $D$ parameter while fixing the \textit{biological factor}  $\lambda:=\frac{\beta}{ \mu}$ , it could be noted that, the "severity" of a disease is better described by the difference $\Delta R_0 (\delta) := \frac{R_0 - R_{c-net}}{\delta}$ rather than $R_0 $ itself. As a direct comparison, compare the epidemic severity among the \autoref{fig:sir_O-PSW_D11_p0.3} and \autoref{fig:sir_O-PSW_D11_p0}. \\
In \autoref{fig:sir_O-PSW_D5_d14_b0.1}, it is underlined how by increasing the transmissibility $ \beta$  also the $ \pi_{max}$ rises nearly to $ 14$ times the homologous plot for $ \beta = 0.015$ (\autoref{fig:sir_O-PSW_D5_p0}).\\
In \autoref{fig:sir_O-PSW_D24_d4}, it is displayed a pathogen faced with medical interventions such that an higher recovery rate $ \mu$ than the COVID-19 one. Even with $D \sim 23$, the epidemic final size $\pi_{max}$  is smaller than the one in \autoref{fig:sir_O-PSW_D11_p0}. In particular, the total cases for $ D = 24, d = 4$ are about $1.5$ times the one for $D = 11, d = 14$.
...
\begin{figure}[p]
	...
	\begin{subfigure}[b]{\linewidth}
        ...
        \caption{SIR spreading for $D = 11$. In the current case, the $R_0 \sim 2.35$ and the differences  $R_0 - R_{c-net} \sim 1.3$ are similar to the ones of \autoref{fig:sir_O-PSW_D11_p0.3}. Yet, the latter has a final outbreak size $\pi_{max} \sim 48\%$ roughly twice as the current $\pi_{max} \sim 20\%$. The \textit{epidemic severity} provides a goods measure of $ \pi_{max}$: the former SIR evolution has $\Delta R_0(\delta) \sim 0.272$ while the current one $ \Delta R_0(\delta) \sim 0.037$.}
	...
	\begin{subfigure}[b]{\linewidth}
        ...
        \caption{SIR spreading for $D = 14$. The current topology ease the diffusion of the pathogen than the Watts-Strogatz regular graph (cf. \autoref{sub@fig:sir_RegLat_D14}).}
        \label{fig:sir_O-PSW_D14_p0}
    \end{subfigure}
	\caption{SIR spreading with COVID-19 parameters and $D = 14$. By zooming into the onset of the outbreak, it could be seen a exponential increase of the total cases. For $D > 14$, the epidemic curves, while develping a more definited exponential growth, start to resemble the mean-field ones.}
...
\begin{figure}[p]
	...
	\begin{subfigure}{\linewidth}
		...
		\caption{A pathogen with more transmissibility than COVID-19 and the other parameters drawn from \autoref{fig:sir_O-PSW_D5_p0}. $\pi_{max}$ rises nearly to $ 14$ times as a product of increasing $ \beta = 0.015$ to $ \beta = 0.1$ (cf. \autoref{fig:sir_O-PSW_D5_p0}).}
	...
	\begin{subfigure}[t]{\linewidth}
		...
		\caption{A pathogen faced with medical interventions such that $\mu$ is higher than the COVID-19 one. As expected, the pharmaceutical interventions plays a relevent role since for $ D = 14$ and $ d = 14$ the final outbreak size overtakes the $40 \% $ of the population while, here, the number of total cases $\pi(t_{max})\sim 13\%$ as $ d = 4$.}
...
\label{sec:res_LROPSW}
The diffusion of the pathogen for $p = 0.3$ shares the overall shape with the $p = 0$ case, but with an enhancement due to the possibility to jump into a new pool of susceptible. In this way, an infected node may have more susceptible around and could spread easily. Moreover, by fixing the epidemic parameters $ \beta \textnormal{ and } \mu$, the shape of the fraction of total cases for average number of contacts $D_{p=0} \sim (1+p) \cdot D_{p=0.3}$ is roughly the same as the one for $D_{p=0.3}$ when the pathogen is neither too strong, reaching all the population instantaneously; nor too weak to decay fast. The \autoref{fig:sir_O-PSW_D11_p0.3} displays this fact compared with \autoref{fig:sir_O-PSW_D14_p0}, i.e. with $14 = D_{p=0} \sim 1.3 \cdot D_{p=0.3}= 11$ . Other way around, at low $D$ the outbreak final size is barely the same (see \autoref{fig:sir_O-PSW_D5_p0}, \autoref{fig:sir_O-PSW_D5_p0.3}). Indeed, it is more easy that the number of infected could reach $ 0$ at random and, thus, end the epidemic. 
In \autoref{fig:sir_O-PSW_D23_d4_p0.3}, there could be seen how a pathogen is suppressed by the PIs measure ( $ d = 4$ ) as $ p=0.3$ and $ D = 24$. In particular, the enhancement is higher than the $p=0$ case (see caption of \autoref{fig:sir_O-PSW_D23_d4_p0.3}).

In addition, when fixing $D, \beta, \mu$, the standard deviation $\sigma(t_{max})$ of the number of total cases for the network is higher for $p = 0.3$ since the pathogen have more possibilities to spread.
...
%This is barely the "growth factor" $ f_{USA}$  obtained by analyzing the JHU dataset for the USA in the linear growth part (see \autoref{fig:USA-AUT-ITAtotalcasesOWID}). 
...
\begin{figure}[htbp]
	...
	\begin{subfigure}{\linewidth}
		...
		\caption{A pathogen spreading on a Caveman Model with $D \sim 2.67$ as yielded by \autoref{eq:CavemanMod_D}. As said for \autoref{fig:sir_RegLat_D2_p0}, this network is a circle with every three nodes clustered into a family. Thus, it shows how by aggregating inviduals prevents the disease to spread undisturbed. This spreading is set for a average number of contacts that slighly overestimates an household size of the european/USA one.}
