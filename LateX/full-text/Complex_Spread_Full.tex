...
\begin{figure}[hbtp]
\centering
\includegraphics[scale=.13]{/home/hal21/MEGAsync/Tour_Physics2.0/Thesis/NetSciThesis/Project/ComplexSpread/LateX/images/unipd_logo.png}
\end{figure}
\vspace{5mm}
\begin{center}
...
As suggested by the \autoref{fig:perfect_model}\footnote{\textit{"Est modus in rebus"} \label{cit:GM}} (\cite{Kiss::MathOfEpiOnNet}), models provide maximum insight where the right balance of realism (circle filled with dots, e.g. regarding epidemiology Graph-Neural-Network to predict $R_t$ \cite{Davahli::USA_predicting_COVID19}) and simplicity (circle with vertical lines e.g. on epidemics the deterministic SIR model) is met\footnote{The equilibrium point may depend on the specific posed question} (central circle with oblique lines).
...
\begin{figure}[ht]
	\centering
	\includegraphics[width = .5\linewidth]{Networks/my_nets/loop_graph.jpg}
	\caption{We labeled the nodes as $1,2,3,4$; while we used the same color for vertexes with the equal numbers of contacts. Node $ 2$ has a loop (black cupola on top).}
	\label{fig:simple}
\end{figure}
...
\begin{figure}[ht]
    \begin{subfigure}{\textwidth}
        \includegraphics[scale = 0.1]{../images/Networks/my_nets/Complete_Graph.png}
        \centering
...
\begin{figure}[ht]
    \begin{subfigure}{\textwidth}
        \includegraphics[scale = 0.2]{../images/Networks/my_nets/Caveman_Model.png}
        \centering
...
In fact, there could be constraints, such as cost-benefit problem, which limit the maximum degree of the bigger node. In those cases, random networks may be the best fit, e.g. national highways network, power grid of generators and switches, $\cdots$
...
\begin{figure}[ht]
    \begin{subfigure}{\textwidth}
        \includegraphics[width = 0.5\linewidth]{../images/Networks/my_nets/Erdoes_Renyi_Graph.png}
        \centering
        \caption{Erdös-Rényi Model for $p = 0.1$}
        \label{fig:ERmodel}
    \end{subfigure}
\end{figure}

The Erdös-Rényi (ER - \autoref{fig:ERmodel}) model was the first attempt to formally describe a random network. In particular, Erdös and Rényi were the fathers of the (random) graph theory, obtained by merging the studies on the networks with Probability subject.
...
\label{sec:SWProp}
A captivating argument, network science allows to study is the "Small-World (SW) phenomenon" or "six degrees of separation". The two quoted names comes, respectively, from a theme developed by the Hungarian writer Fridgyes Karinthy in a play called "Chains" ($1978$) and an empirical experiment injected by the psychologist S. Milgram in $1967$. Its famous version states that an actor/actress, e.g. Kevin Bacon, is linked to a random actor/actress, e.g. Andrea Scotti, through only few acquaintances: according to \href{https://oracleofbacon.org/movielinks.php}{Kevin Bacon Number}\footnote{https://oracleofbacon.org/movielinks.php} only by $2$ persons in common. By extending this idea to real networks \cite{barabasi::2016networkbook}, it is possible to recover that the distance between two randomly chosen nodes is small. Random networks account for this phenomenon as we will deepen.
...
\end{itemize}
This is in an one-to-one relation with a First-Order-Phase Transition of Statistical Mechanics. The formation of an ordered giant component could be compared with ice formation: the passage from disordered water molecules to an ordered structure.
...
Furthermore, $C/\langle k \rangle \propto N \quad \textnormal{ if } N >> 1$ and $ C/ \langle k \rangle$  is higher than its theoretical expectation. Thus, a refined model is needed to explain the big \textit{cluster coefficient} for small probability $ p$ (or high $N$).
...
To face the problem of the "high $C$ at low edge probability $p_{ER}$" (we redefined the old $p$ as $p_{ER}$), while preserving the "Small-World", Duncan Watts and Steven Strogatz proposed a hybrid model which can interpolated between a regular lattice for $p = 0$ to a random network model for $p = 1$. The new borne model is, thus, called "Watts-Strogatz (WS) model".
Practically, one starts by generating a grid-like network with $\langle k \rangle$ neighbors for each node. Then, one fixes a node and for each edge rewire, with probability $ p$, its \textit{target} node with a random vertex. Lastly, one iterates rewiring procedure over all the exhisting edges.
Therefore, the \textit{degree distribution} goes from a peaked histogram of the regular lattice to a Poissonian distribution. Meanwhile the mean $ D$ remains roughly the same and no \textit{hubs} are generated.
A further constraint is that no multiple edges can connect the same pair of vertexes, i.e. no multi-edges. The presence of multiedges gives a correction of $1/N$ for big $N$, negligible for big $N$.
...
It worths to highlight that the \textit{rewiring probability} $p$ accounts only for "long-range" connections, while $p_{ER}$ provides either local edges ("short range") and distant interactions ("long range"). Thus, the \textit{distant} vertexes are, on average, $p\cdot L$ where $L$ is the total number of links.
...
Another method to generate a network \textit{from a fixed degree distribution} is the "hidden parameter ($\eta$) method" which do not form multi-edges and loops \cite{barabasi::2016networkbook} and allows to tune the average degree $\langle k \rangle$ modifying the parameter $\eta$.
...
In the following sections, we will use only the Configuration Model.
...
\begin{figure}[ht]
    \begin{subfigure}{.45\linewidth}
        \includegraphics[width = \linewidth]{Networks/my_nets/NNO_Conf_Model_AdjMat_1000_25.0_0.0.png}
        \centering
	...
	\hfill
	\begin{subfigure}{.45\linewidth}
        \includegraphics[width = \linewidth]{Networks/my_nets/NN_Conf_Model_AdjMat_1000_21.0_0.0.png}
        \centering
        \caption{Sparse PSW degree distribution}
        \label{fig:netmod_S-PSW}
    \end{subfigure}
	\caption{Two kind of PSW network account for "Overlapping" and "Sparse" structures. The graph realization (upper left) has $ N = 100$ for a better visualization, we fixed $ N = 1000$ the adjaciency matrix (upper right) and the degree distribution (lower band). We do not report the realizations with $p = 0.3$, since it could be straightforwardly obtained by the superimposing some long-range links to the current cases as in \autoref{fig:net_RegLat_D500_p0.3}.}
\end{figure}
...
\begin{figure}[ht]
	\includegraphics[width = .5\linewidth]{Networks/my_nets/Barabasi_Albert_Model.png}
	\centering
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{Results/WS_Pruned/p0.3/WS_Pruned_SIR_R0_3_N1000_D2.0_p0.3_beta0.105_d14.0.png} 
    ...
    \begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{Results/WS_Pruned/p0.3/WS_Pruned_SIR_R0_3_N1000_D6.0_p0.3_beta0.035_d14.0.png} 
    ...
    \begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{Results/WS_Pruned/p0.3/WS_Pruned_SIR_R0_3_N1000_D14.0_p0.3_beta0.015_d14.0.png} 
...
\begin{figure}[H]
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width = \linewidth]{Results/BA_Model/p0.0/Pec_Beh/BA_Model_SIR_R0_8_N1000_D6.0_p0.0_beta0.1_d14.0.png}
	...
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width = \linewidth]{Results/BA_Model/p0.0/Pec_Beh/BA_Model_SIR_R0_1_N1000_D22.0_p0.0_beta0.015_d4.0.png}
