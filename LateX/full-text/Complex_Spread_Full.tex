...
\begin{equation}
	...
	\label{eq:RcRnet_picrit_network}
\end{equation}
...
Lastly, in the following discussion, we will underline only the relevant features the network topologies could provide. Therefore, a proper way to face this chapter would be to firstly dive into \autoref{ch:Methodology} for a more general discussion on the introduced quantities and behaviors. 
...
\end{figure}
The goal of this section is to test whether the \textit{basic reproduction number} $R_0$ is a suitable quantity to estimate the "strength" of an epidemic among different network models. In fact, $R_0$ represents only the number of secondary cases in the early time-steps, e.g. at the next day after. 
...
The work-flow is the following. We generate a fully-connected graph using a "Watts-Strogatz" (WS) algorithm with $D = 999, p=0$. This model is, thus, called "Regular Lattice Graph". Then, the core idea is to fix $ R_0 $ and visualize how the epidemic curves change by varying the transmissibility $ \beta$ and $ D$. In particular, we fixed the recovery rate $ \mu = 1/d$ and, then, dividing the average number of contacts $ D$ while doubling $\beta$. Formally,
\begin{equation}
...
\textit{network threshold} $R_{c-net} = (1-D^{-1})^{-1}$ decreases when $D$ increases. As expected, $R_{c-net}$ is more likely to be overtaken when a regular lattice has more neighbors.
...
Finally, in \autoref{fig:sir_RegLat_D30}, it is possible to recover that when the pathogen is favored by the network structure, also the estimated maximum of $ i(t)$ ( $ \bigstar$) starts to resemble the mean-field one.
...
\begin{figure}[H]
	...
	The pathogen, being favored by the network structure, drives to an estimated maximum of $ i(t)$ ( $ \bigstar$) nearly the mean-field one (yellow star).}
...
As $D=14$ \autoref{fig:RegLat_D14_p0.3} or $ D = 2$, the fluctiation are smaller than the intermediate case for $ D = 6$.   An approximate explanation is that the the population size $N$ and the lower bound of $0$ infected even out the trajectories as it is briefly expressed in \autoref{ch:Methodology}: wheter a pathogen has an "intermediate strength", between the mean-field and a fast decay evolution, it could diffuse without that upper and lower bounds. Hence, the trajectories display broater scenarios, which translate into an higher fluctuation over the mean $ \sigma(t_{max})$ . 
...
As in \autoref{sec:RegLat_p0}, by varying the $D$ parameter while fixing $\lambda:=\frac{\beta}{ \mu}$ , it could be noted that, the "severity" of a disease is better described by the difference $\Delta R_0 (\delta) := \frac{R_0 - R_{c-net}}{\delta}$ rather than $R_0 $ itself. As a direct comparison, compare the epidemic severity among the \autoref{fig:sir_O-PSW_D11_p0.3} and \autoref{fig:sir_O-PSW_D11_p0}. In the current case, the $R_0 \sim 2.35$ and the differences  $R_0 - R_{c-net} \sim 1.3$ are simlar while \autoref{fig:sir_O-PSW_D11_p0.3} reaches a final outbreak size $\pi_{max} \sim 48\%$ twice as the current $\pi_{max} \sim 20\%$ of \autoref{fig:sir_O-PSW_D11_p0}. Yet, the former SIR has $R_0(\delta) \sim 0.49$ while the latter $ R_0(\delta) \sim 0.083$. This results shows how the introduced parameter $\Delta R_0 (\delta)$ could better account for the "epidemic severity".\\
...
\begin{figure}[p]
	...
	\begin{subfigure}[b]{\linewidth}
        ...
        \caption{SIR spreading for $D = 11$. In the current case, the $R_0 \sim 2.35$ and the differences  $R_0 - R_{c-net} \sim 1.3$ are simlar to the ones of \autoref{fig:sir_O-PSW_D11_p0.3}. Even if displaying similar parameters, the latter has a final outbreak size $\pi_{max} \sim 48\%$ roughly twice as the current $\pi_{max} \sim 20\%$. Yet, the former SIR evolution has $R_0(\delta) \sim 0.49$ while the latter $ R_0(\delta) \sim 0.083$.}
...
\begin{center}
	\begin{tabular}{||c|c||}
		...
		\hline
		\multicolumn{1}{|c|}{$ \mu$ [1/time-step] } & \multicolumn{1}{c|}{days [time-step]}\\
		\hline
		0.071 &  14\\ 
		\hline
		0.111 & 9 \\
