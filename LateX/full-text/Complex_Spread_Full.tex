...
In order to analyze the linear behavior of $P(t)$, it is worth defining \textit{critical basic reproduction number} the  $R_{0c} := \beta D_c / \mu \stackrel{!}{=} R_{c-net}$ and in the case where the network is not so heterogeneous $ R_{c-net} \sim 1$, where $1$ is for a poissonian distribution. This is, indeed, the case for the reported networks (\autoref{ch:Results}) yielding a critical degree $D_c = \frac{\mu}{\beta}$. 
...
\begin{enumerate}
	...
	\begin{itemize}
		\item fisso $\lambda$ vario $D \& p \Rightarrow \textnormal{ variano i contagi}$ 
		\item fisso $\lambda$ vario $ \mu \textnormal{ (vaccino)} \Rightarrow \textnormal{ variano i contagi}$ 
		\item stesso $R_0 $ con $ p \neq p' \textnormal{ e } p = p'$, epidemia varia;
		\item mostra che se $R_0 < R_{c-Net} < 1$ l'epidemia cresce;
		\item $\sigma_D(p \uparrow) \sim \uparrow$ for Overlapping PSW?     
		\end{itemize}
...
\begin{figure}[h]
	\centering
	\includegraphics[width = .7\linewidth]{Results/WS_Pruned/AdjMat/WS_Pruned_AdjMat_1000_500.0_0.0.png}
...
\begin{figure}[t]
	\centering
	\includegraphics[width = .5\linewidth]{Results/WS_Pruned/AdjMat/WS_Pruned_AdjMat_1000_500.0_0.1.png}
...
\begin{figure}[p]
    ...
    \begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=.97\textwidth]{Results/WS_Pruned/p0.1/WS_Pruned_SIR_R0_3_N1000_D6.0_p0.1_beta0.083_mu0.2.png} 
    ...
    \begin{subfigure}[t]{\textwidth}
        \centering
        \includegraphics[width=.97\textwidth]{Results/WS_Pruned/p0.1/WS_Pruned_SIR_R0_2_N1000_D14.0_p0.1_beta0.036_mu0.25.png} 
...
\begin{figure}[ht]
    \begin{subfigure}{.45\linewidth}
        \includegraphics[width = \linewidth]{Networks/NNO_Conf_Model_addE_True_AdjMat_1000_14.0_0.0.png}
        \centering
	...
	\hfill
	\begin{subfigure}{.45\linewidth}
        \includegraphics[width = \linewidth]{Networks/NN_Conf_Model_AdjMat_1000_13.0_0.0.png}
        \centering
...
\begin{figure}[p]
    ...
    \end{subfigure}
	\caption{SIR spreading with COVID-19 parameters}
...
\begin{figure}[p]
	...
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width = .90\linewidth]{Results/PSW/OPSW/p0.0/Pec_Behaviours/NNO_Conf_Model_addE_True_SIR_R0_2_N1000_D7.0_p0.0_beta0.05_mu0.167.png}
		\caption{A pathogen with faster human-recovery but more infectivity than the COVID-19 one. $\Delta R_{0}|_{d=4} > \Delta R_{0-COVID-19}$, rather, $\Delta R_0 (\delta)|_{d=4} \lesssim \Delta R_0 (\delta)|_{COVID-19}$ could be considered a more concrete mark of the epidemic bourden.}
		\label{fig:sir_O-PSW_D7_d4}
	\end{subfigure}
	\vfill
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width = .90\linewidth]{Results/PSW/OPSW/p0.0/Pec_Behaviours/NNO_Conf_Model_addE_True_SIR_R0_1_N1000_D24.0_p0.0_beta0.015_mu0.25_SEASINFLUENZA.png}
		\caption{A pathogen faced with medical interventions such that $\mu$ is lower than the COVID-19 one. Even with $D$ enhanched to barely $23$, the epidemic final size is less of \autoref{fig:sir_O-PSW_D11_p0}}
		\label{fig:sir_O-PSW_D23_d4}
	\end{subfigure}
	\vfill
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width = .90\linewidth]{Results/PSW/OPSW/p0.0/Pec_Behaviours/NNO_Conf_Model_addE_True_SIR_R0_1_N1000_D24.0_p0.0_beta0.05_mu1.0.png}
		\caption{The sub-exponential growth for a $D$ slightly above the $D_c$ of \autoref{fig:Ordp_OPSW_highmu_COVID19}}
		\label{fig:sir_O-PSW_D24_d1}
	\end{subfigure}
...
\begin{figure}[p]
	...
	\begin{subfigure}[t]{\linewidth}
		...
		\caption{A pathogen spreading with higher $\mu$ than the COVID-19 one. $\pi(t_{max})$ is similar to the one for $D_{p=0} \cdot p \sim 14$ \autoref{fig:sir_O-PSW_D14_p0}. Note how, if limited at the first hundred days, the epidemic produce a total number of infected increasing, roughly, linearly in time. This is a commond feature for $D < D_c$.}
		\label{fig:sir_O-PSW_D11_p0.3}
	\end{subfigure}
	\begin{subfigure}[t]{\linewidth}
		\centering
		\includegraphics[width = .90\linewidth]{Results/PSW/OPSW/p0.3/Pec_Behaviours/NNO_Conf_Model_addE_True_SIR_R0_5_N1000_D1.0_p0.3_beta0.4_mu0.07.png}
		\caption{A pathogen with same recovery rate $\mu$ but more infective than the COVID-19 one. $\pi(t_{max})$ is similar to the one for $D_{p=0}$.}
		\label{fig:sir_O-PSW_D111_p0.3}
	\end{subfigure}
...
\begin{figure}[t]
	...
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth, keepaspectratio]{Results/PSW/OPSW/OrdParam/p0.0/NNO_Conf_Model_addE_True_ordp_p0.0_beta0.05_mu1.0.png}
		\caption{Order Parameter exhibiting a higher $D_c$ since recovery rate is enhanched more than the transmissibility}
		\label{fig:Ordp_OPSW_highmu_COVID19}
	\end{subfigure}
	\caption{Order Paramater with different recovery rates at $p = 0$ }
\end{figure}
...
\begin{figure}[t]
	...
	\end{subfigure}
	\begin{subfigure}{0.48\linewidth}
		\includegraphics[width=\linewidth, keepaspectratio]{Results/PSW/OPSW/OrdParam/p0.3/NNO_Conf_Model_addE_True_ordp_p0.3_beta0.05_mu1.0.png}
		\caption{Higher $D_c$ since recovery rate is higher than the transmission rate}
		\label{fig:Ordp_OPSW_highmu_p0.3}
	\end{subfigure}
	\caption{Long Range Order Parameter of different pathogens and recovery rates.}
\end{figure}
...
\begin{figure}[ht]
	\includegraphics[width = \linewidth]{Results/PSW/SPSW/p0.0/Pec_Behaviors/NN_Conf_Model_SIR_R0_2_N1000_D5.0_p0.0_beta0.1_d4.0.png}
	\caption{(pseudo-)Linear growth of the number of total cases extending for more than 20 time-steps.}
\end{figure}
...
\begin{figure}[p]
	...
	\begin{subfigure}{\linewidth}
		...
		\caption{The evolution for a larger number of contact $D = 10$}
		\label{fig:sir_S-PSW_D10_p0.0}
	\end{subfigure}
	\vfill
	\begin{subfigure}{\linewidth}
		\centering
		\includegraphics[width = 0.9\linewidth]{Results/PSW/SPSW/p0.0/NN_Conf_Model_SIR_R0_3_N1000_D13.0_p0.0_beta0.015_d14.0.png}
		\caption{The evolution for a larger number of contact $D = 14$. From this $D$ on, the epidemic growth is going to develop an exponential beavior.}
		\label{fig:sir_S-PSW_D13_p0.0}
	\end{subfigure}
...
\newpage
\subsubsection*{Long Range S-PSW}
For brevity, it is shown only a SIR evolution (\autoref{fig:sir_SPSW_p0.3_D3}) where the linearity is supported by the "long-range" interactions as in the cases seen before. All the "linear" curves share a number of total cases approximatevely from $10\% - 20\%$. Therefore, by characterizing this interval with $R_0(\delta)$ it could be possible to predict this linear trend from the initial conditions, i.e. $D, \beta, \mu, \delta, \langle k^2 \rangle$.

\begin{figure}[h]
	\includegraphics[width = \linewidth]{Results/PSW/SPSW/p0.3/Pec_Beh/NN_Conf_Model_SIR_R0_2_N1000_D3.0_p0.3_beta0.05_d14.0.png}
	\caption{(pseudo-)Linear growth of the number of total cases extending barely for 100 time-steps.}
	\label{fig:sir_SPSW_p0.3_D3}
\end{figure}


\newpage
\subsubsection*{Order Parameters}
In this subsection, the evolution of $SD(C(t))$ driven by $D$. As the previous case, $D_{c-homog model} < D_{c-ER model} < D_{c-fuse model}$. On the other hand, even at a plain eye inspection the \autoref{fig:Ordp_OPSW_COVID-19} the difference in $D_c$ is remarkable. More specifically, for the COVID-19 parameters, it could be seen that the departure from a ps-linearity is expected at $D \sim 10$ considering a SPSW; while in the homologous plot 

\textbf{Regular S-PSW Order Parameter} 
\begin{figure}[t]
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth, keepaspectratio]{Results/PSW/SPSW/OrdParam/p0.0/NN_Conf_Model_ordp_p0.0_beta0.015_d14.0.png} 
		\caption{Order Parameter with fixed COVID-19 parameters}
		\label{fig:Ordp_SPSW_COVID-19}
	\end{subfigure}
	\begin{subfigure}{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth, keepaspectratio]{Results/PSW/SPSW/OrdParam/p0.0/NN_Conf_Model_ordp_p0.0_beta0.05_d1.0.png}
		\caption{Order Parameter exhibiting a higher $D_c$ since $\beta$ and $ \mu$ are changed.}
		\label{fig:Ordp_SPSW_d1.0_COVID19}
	\end{subfigure}
	\caption{Order Paramater for SPSW at different epidemic rates at $p = 0$ }
\end{figure}
